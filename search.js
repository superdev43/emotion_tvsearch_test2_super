...
function setButton() {
    ...
    console.log('flagsearchPage->', flagSearchPage)
    if (flagSearchPage) {
...
function onSubmit() {
    ...
    // localStorage['tv_setuptype'] = document.getElementById('tv_setuptype').value;
    // localStorage['tv_seriestype'] = document.getElementById('tv_seriestype').value;
...
function search() {
    ...
    // ■説明
    // 検索ページでは、下記２つの select が存在し、常に連動しており、検索条件を変えると２つ同時に変化する。
    // 検索ページ以外からの検索実行時は、GETによってURLに値を入れて、検索ページに遷移する。
    // 遷移後にjavascriptによってURLから値を取得するためGETにしている。
    // その為、tv_seriestype の長い文字列となる値を使いたくないので、
    // 「tv_seriestype2」の値をGETで渡し、遷移後、受け取った値から、text部分の「上下の角度調節機能が欲しい」などを取得し、
    // 今度は「tv_seriestype」から、取得してtext部分が一致した値「OP117-S PLB-A...」などを取得する様にしている。
    // 【1】tv_seriestype2
    // <select name="tv_seriestype" id="tv_seriestype" style="width:150px" onchange="setButton();">
    //   <option value="">全シリーズ</option>
    //   <option value="1">上下の角度調節機能が欲しい</option>
    //   <option value="2">角度固定で薄く設置したい</option>
    //   <option value="3">アームで上下左右に向かせたい</option>
    // </select>
    // 【2】tv_seriestype（hidden）
    // <select name="" id="tv_seriestype2" style="display:none">
    //   <option value="">全シリーズ</option>
    //   <option value="OP117-S PLB-ACE-117M PLB-ACE-117S PLB-ACE-228M PLB-ACE-228S PLB-ACE-148S LCD-ACE-113 ">上下の角度調節機能が欲しい</option>
    //   <option value="PLB-ACE-104M PLB-ACE-104S PLB-ACE-141S SPK-TPS-2S ">角度固定で薄く設置したい</option>
    //   <option value="A4041 AE222 AE422 AE444 PRM-LT25M PRM-LT17M ...省略">アームで上下左右に向かせたい</option>
    // </select>

    // 初期化
    var kanaguid = '';
    // tv_seriestype の値を取得する
    var tv_seriestype_no = document.getElementById('tv_seriestype').value;

    // 取得した値のテキストを取得する
    var tv_seriestype_text = '';
    if (tv_seriestype_no) {
        // 取得した値のテキストを取得する
        var obj = document.getElementById('tv_seriestype');


        for (var i = 0; i < obj.length; ++i) {
            if (tv_seriestype_no == obj[i].value) {
                tv_seriestype_text = obj[i].text;
                break;
            }
        }
        // 金具IDのプルダウン（tv_seriestype2/hidden）を検索し、
        // 取得した値のテキストと一致する値（金具ID）を取得する
        var obj = document.getElementById('tv_seriestype2');
        for (var i = 0; i < obj.length; ++i) {
            if (obj[i].text == tv_seriestype_text) {
                kanaguid += obj[i].value + " ";
            }
        }
    } else {
        // tv_seriestype の値が無い場合は、
        // 金具IDのプルダウン（tv_seriestype2/hidden）の全データを取得する
        var obj = document.getElementById('tv_seriestype2');
        console.log('obj->', obj)
        for (var i = 0; i < obj.length; ++i) {
            kanaguid += obj[i].value + " ";
    ...
    // $('#new-header > img').hide();
    console.log("ddddasddd->", itemlist)
